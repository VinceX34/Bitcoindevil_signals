...
// app/api/worker/process-ch-queue-ai/route.ts
...
export async function GET() {
  const workerRunId = `ai-${Math.random().toString(36).substring(7)}`;
  console.log(`[Worker AI RUN ${workerRunId}] Starting run.`);
  let tasksProcessedThisRun = 0;

  try {
    // Check global rate limit status first
    if (!await canMakeApiCall()) {
      console.log(`[Worker AI RUN ${workerRunId}] Global rate limit active, skipping this run.`);
    ...
    for (let i = 0; i < MAX_TASKS_PER_WORKER_RUN; i++) {
      const taskResult = await executeTransaction(async (executeQueryInTransaction) => {
        const pendingTasks = await executeQueryInTransaction(
          `SELECT * FROM cryptohopper_queue_ai 
           WHERE (
        ...
        await executeQueryInTransaction(
          `UPDATE cryptohopper_queue_ai SET status = 'processing', attempts = attempts + 1, last_attempt_at = NOW() WHERE id = $1`,
          [taskToProcess.id]
        );
      ...
      if (!taskResult.processed || !taskResult.task) {
        console.log(`[Worker AI RUN ${workerRunId}] No more tasks to process.`);
        break;
      }
      ...
      await executeQuery(
        `INSERT INTO forwardedsignals_ai (tradingview_signal_id, task_sub_id, http_status_code, tradingview_payload, cryptohopper_payload, cryptohopper_response, status, error_message, hopper_id, exchange_name)
      ...
      await executeQuery(
        `UPDATE cryptohopper_queue_ai SET status = $1, error_message = $2 WHERE id = $3;`,
        [finalQueueTaskStatus, chApiError, currentTask.id]
      );
      console.log(`[Worker AI RUN ${workerRunId}] Processed task ${currentTask.id}. Final status: ${finalQueueTaskStatus}.`);
    }
    console.log(`[Worker AI RUN ${workerRunId}] Loop finished. Processed ${tasksProcessedThisRun} task(s).`);
    return NextResponse.json({ success: true, message: `Worker AI run ${workerRunId} completed.` });
  } catch (workerError: any) {
    console.error(`[Worker AI RUN ${workerRunId}] Unhandled error:`, workerError);
    return NextResponse.json({ success: false, error: 'Worker AI failed.', details: workerError.message }, { status: 500 });
  }
} 